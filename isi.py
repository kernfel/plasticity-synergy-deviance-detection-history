...
N_networks = 10
ISIs = (100, 200, 300, 400, 500, 750, 1000, 1250, 1500)
    ...
    for std, tau_rec_ in enumerate((0*ms, params['tau_rec'])):
        for ta, th_ampl_ in enumerate((0*mV, params['th_ampl'])):
            ...
            for i, isi in enumerate(ISIs):
                ...
                all_results, T = readout.setup_run(Net, mod_params, rng, {key: j for j, key in enumerate('ABCDE')})
                Net.run(T)
                readout.get_results(Net, mod_params, W, all_results)
                readout.compress_results(all_results)
                try:
                    dd.io.save(fname.format(**locals()), dict(all_results=all_results, params=mod_params, X=X, Y=Y, W=W, D=D))
            ...
            print(f'Completed ISI sweep {(net+1)*(std+1)*(ta+1)}/{N_networks*4} after {(time.time()-T)/60:.1f} minutes.')

    fig, axs = plt.subplots(2, figsize=(9,12))
    for ax, idx, label in zip(axs, (ddi, ai), ('Deviance detection', 'Adaptation')):
        ax.plot(ISIs, idx[:, 0, 0, :net+1].mean(0), label='No plasticity')
        ax.plot(ISIs, idx[:, 1, 0, :net+1].mean(0), label='Short-term depression')
        ax.plot(ISIs, idx[:, 0, 1, :net+1].mean(0), label='Threshold adaptation')
        ax.plot(ISIs, idx[:, 1, 1, :net+1].mean(0), label='Both STD and TA')
        ax.plot(ISIs, idx[:, 0, 1, :net+1].mean(0) + idx[:, 1, 0, :net+1].mean(0), '--', label='Summation')
