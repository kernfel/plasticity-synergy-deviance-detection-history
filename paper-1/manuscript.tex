...
To show this in the sample network, we correlated the number of spikes in dev target trials with the spike-triggered depression variable $x_j$ at the start of later trials (cf. Equation \ref{eqn-xsyn}) across excitatory neurons. We found that, while the correlation was very clear in the trial immediately following target (Pearson's \rho = -0.984, p = 0), it was almost completely lost by the next trial (\rho = -0.0525, p = 0.138) under the influence of both decay and unrelated activity in the intervening trial. In fact, even without intervening activity, inputs to $x_j$ would decay to a negligible $e^\frac{1 s}{\tau_x} = 0.0013$ within two trials.
Thus, STD could not have any stimulus-specific effects across dev target trials or msc trials.

However, given the substantial correlation from one trial to the next, the frequently presented stimulus B could have experienced stimulus-specific adaptation as a result of STD. Referring back to Figure \ref{fig3}b, we see that there was indeed a significant, if small, effect of presentation frequency even in the ablated model with STD only, yielding a positive SSA index on average.

To show that STD carry-over was responsible for the reduction of non-target dev pulses relative to msc, we needed to estimate the extent to which activity of a given neuron (or, alternatively, its membrane potential $V_m$) is affected by changes in STD. Unlike $V_{TA}$, which directly affects the neuron to which the value belongs, $x_j$ exerts its effects at the other side of a synapse. Therefore, we devised a measure $D$ to estimate the postsynaptic impact of STD in terms of membrane potential lost. $D$ is necessarily activity-dependent -- a postsynaptic neuron's membrane potential is driven only by presynaptic partners that fire -- so must be calculated based on observed activity. Here, we used the average trial response $R_j^S$ of neuron $j$ to a given stimulus $S$, in number of spikes, as the presynaptic drive. Before depression, a single excitatory presynaptic spike evokes a postsynaptic potential (EPSP) peaking at approximately $k = 1.4$ mV at rest (recall that $w = 1$ for all synapses), and STD linearly scales this EPSP by presynaptic $x_j$. Thus, ignoring added depression due to repeated spikes within a trial, and instead using the average depression at the start of trials $x_j^S$, we estimated the average peak EPSP evoked by a presynaptic neuron $j$ as $R_j^S k x_j^S$. Finally, with $x_j \in [0,1]$ and summing over presynaptic neurons, we could estimate the peak EPSP lost to depression in a given neuron $i$ as
\begin{equation}
    D_i^S = \sum_{j \in pre} R_j^S k (1-x_j^S)
\end{equation}
Notice that $D$ is measured in volts due to scaling with $k$, and is therefore roughly comparable to $V_{TA}$, albeit more approximate.
...
\textbf{b} Statistics across networks and stimuli of the contrast (dev - msc) in $D$ ($D^B$ is shown first, since all other data in this panel relate directly to A), and in $V_{TA}$ at the start of A trials (cf. Figure \ref{fig7}d). In each network, averages were taken over neurons active (>0.2 spikes/trial) in either dev or msc trials (B trials for $\Delta D^B$, A trials for $\Delta D^A$ and $\Delta V_{TA}$). Whereas the third box ($\Delta V_{TA}$ total) indicates averages in the full model, the final box ($\Delta V_{TA}$ added) indicates the difference between the full and the ablated (TA only) model
...
In Figure \ref{fig8}a, we see that target trials (top) experienced depression across large parts of the network in both conditions. While there were differences between the two conditions, they were relatively small and did not appear to follow a readily identifiable pattern. Conversely, B trials (bottom) were spatially more clustered around the stimulation site in both conditions. In the msc sequence, we can attribute the difference between A and B primarily to the response size: The context was the same, but the B response tended not to spread out as much in this network. However, in the dev sequence, the B response was limited almost exclusively to a very local patch around the stimulation site (see also Figure \ref{fig6}a), with depression, of course, similarly limited. As a result, the contrast (dev - msc) shows slightly reduced $D$ in the periphery, but strikingly elevated $D$ near the stimulation site, indicative of increased synaptic depression in dev.
