...
\end{equation}
We define related indices, contrasting the responses in msc against std, and in dev against std, analogously. Since contrasts of both kinds have been called "stimulus-specific adaptation" or SSA in the literature \citep{Kubota2021-dx,Taaseh2011-gg}, we chose to avoid such ambiguity, using instead the term "repetition suppression" (RSI, $R^A_{msc} - R^A_{std}$) to refer to the reduction in response induced mainly by a difference in presentation frequency while controlling for context, and the term "mismatch" (MMI, $R^A_{dev} - R^A_{std}$) to refer to the uncontrolled contrast, which captures a combination of repetition suppression and deviance detection.
...
We could readily identify similar patterns emerging across most networks and stimulus assignments. To show that deviance detection is robust to network and stimulus identity, we calculated the DDI and related indices mentioned above for each network, and show them in a summary plot in Figure \ref{fig2}c. We found that our model reliably produced positive mismatch indices (T = 6.67e+03, p = 9.26e-16, median 0.253; one-sided Wilcoxon signed-rank test, n = 120) while both RSI (T = 6.19e+03, p = 9.76e-12, median 0.135) and DDI (T = 5.97e+03, p = 4.75e-10, median 0.114) were also positive, despite some outliers. This shows that our model was well behaved and suitable for an investigation of deviance detection.
...
The RSI (Figure \ref{fig3}b) was exactly 0 as expected in the ablation with no plasticity, and positive with any plasticity applied (STD only: T = 4.97e+03, p = 0.000222, median 0.0039; TA only: T = 5.99e+03, p = 3.3e-10, median 0.097; STD+TA: T = 6.19e+03, p = 9.76e-12, median 0.135; one-sided Wilcoxon signed-rank tests, n = 120). Since the RSI measures the degree of adaptation to frequent stimulus presentation, this indicates that both plasticity mechanisms were capable of imbuing the networks with some level of history dependence.
